...
type MeResponse = {
  ...
  data: {
    ...
    profile: {
      municipality: { id: string; name: string; state: string; centerLat?: number | null; centerLng?: number | null } | null
    }
  }
}
...
export default function GestorPerfilPage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      setMunName(json.data.profile.municipality?.name || "")
      setMunState(json.data.profile.municipality?.state || "")
      setCenterLat(json.data.profile.municipality?.centerLat?.toString?.() || "")
      setCenterLng(json.data.profile.municipality?.centerLng?.toString?.() || "")
    } catch (e: any) {
  ...
  const onPickAvatar = async (file: File) => {
    try {
      const signRes = await fetch("/api/uploads/cloudinary/sign", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ resourceType: "image" }) })
    ...
    try {
      const payload = {
        ...
        municipality: {
          name: munName,
          state: munState,
          centerLat: centerLat.trim() ? Number(centerLat) : undefined,
          centerLng: centerLng.trim() ? Number(centerLng) : undefined,
        },
      }
      ...
      <Card className="p-6 space-y-6">
          ...
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              ...
              <Button asChild type="button" variant="outline" className="bg-transparent">
                <label htmlFor="avatar-upload-m" className="cursor-pointer">
                  Trocar foto
                </label>
              </Button>
              <Button
        ...
        <div className="border rounded-lg p-4 space-y-4">
          ...
          <div className="grid md:grid-cols-2 gap-4">
            <div className="space-y-2">
              <Label>Nome</Label>
              <Input value={munName} onChange={(e) => setMunName(e.target.value)} />
            </div>
            <div className="space-y-2">
              <Label>UF</Label>
              <Input value={munState} onChange={(e) => setMunState(e.target.value)} />
            </div>
          </div>
          <div className="grid md:grid-cols-2 gap-4">
            <div className="space-y-2">
              <Label>Centro do mapa (lat)</Label>
              <Input value={centerLat} onChange={(e) => setCenterLat(e.target.value)} placeholder="-15.78" />
            </div>
            <div className="space-y-2">
              <Label>Centro do mapa (lng)</Label>
              <Input value={centerLng} onChange={(e) => setCenterLng(e.target.value)} placeholder="-47.93" />
            </div>
          </div>
        </div>
